...
type Tab = 'conversation' | 'vocabulary' | 'qna' | 'image' | 'translate';
...
const App: React.FC = () => {
  ...
  const NavButton = ({ tabName, label, icon }: { tabName: Tab; label: string; icon: React.ReactNode }) => (
    <button
      ...
      className={`group flex-1 relative flex flex-col items-center justify-center gap-1 px-2 py-3 rounded-2xl transition-all duration-300 ease-out
        ${activeTab === tabName 
            ? 'text-white' 
  ...
  return (
    <div className="flex flex-col h-screen font-sans bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden selection:bg-violet-200 dark:selection:bg-violet-900/50 relative">
      ...
      <main className="flex-1 overflow-hidden p-3 sm:p-6 relative z-10">
        <div className={`max-w-5xl mx-auto h-full flex flex-col bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[2rem] shadow-2xl shadow-slate-200/50 dark:shadow-slate-950/50 border border-white/50 dark:border-slate-800 overflow-hidden ring-1 ring-slate-900/5 dark:ring-white/5 ${animationClass}`}>
          {renderTabContent()}
        </div>
      </main>
      ...
      <footer className="p-4 pb-6 bg-transparent z-20">
        <nav className="max-w-3xl mx-auto flex items-center justify-between gap-1 p-1.5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-slate-400/20 dark:shadow-black/40 border border-white/50 dark:border-slate-800 ring-1 ring-slate-900/5">
